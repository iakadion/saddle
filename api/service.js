...
export function saddleservice(options = {}) {
  ...
  async function handle(request) {
    ...
    try {
      ...
      if (url.pathname === "/v1/scrape" && request.method === "POST") { assertpublicurl(input.url, options.security); const result = await options.scrape(input.url, input); return jsonresponse(result); }
