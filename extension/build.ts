...
const rootpath = existsSync(resolve(modulepath, "..", "..", "package.json"))
  ? resolve(modulepath, "..", "..")
  : resolve(modulepath, "..");
const entries = [
  "manifest.json",
...
async function resolveentry(entry) {
  ...
  try {
    await access(javascript);
    return javascript;
  } catch {
    return resolve(rootpath, "extension", entry.replace(/\.js$/, ".ts"));
  }
}
...
export async function buildextension(options = {}) {
  ...
  const manifest = JSON.parse(
    await readFile(resolve(rootpath, "extension/manifest.json"), "utf8"),
  );
