...
export async function fetchRobotsTxt(siteUrl: string, options: { timeout?: number; userAgent?: string } = {}): Promise<RobotsTxt> { const base = siteUrl.replace(/\/$/, ""); const robotsUrl = `${base}/robots.txt`; const cached = cache.get(robotsUrl); if (cached && Date.now() - cached.fetchedAt < cachettl) return cached.robots; try { const response = await fetch(robotsUrl, { headers: { "User-Agent": options.userAgent ?? "Saddle/1.8.10" }, signal: AbortSignal.timeout(options.timeout ?? 5000) }); const robots = response.ok ? robotsrules(await response.text()) : robotsrules(); cache.set(robotsUrl, { robots, fetchedAt: Date.now() }); return robots; } catch { const robots = robotsrules(); cache.set(robotsUrl, { robots, fetchedAt: Date.now() }); return robots; } }
