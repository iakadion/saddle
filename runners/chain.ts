...
function normalizeproviders(input) {
  ...
  return Object.freeze(input.map((value, index) => {
    ...
    return Object.freeze({ id, priority: nonnegative(value.priority ?? index, "provider chain provider priority"), status, capabilities: Object.freeze(unique(value.capabilities ?? [], "provider chain capability")), cpu: nonnegative(value.cpu ?? 0, "provider chain provider cpu"), memorybytes: nonnegative(value.memorybytes ?? 0, "provider chain provider memorybytes"), maxmilliseconds: nonnegative(value.maxmilliseconds ?? 0, "provider chain provider maxmilliseconds") });
  }).sort((left, right) => left.priority - right.priority || left.id.localeCompare(right.id)));
}

function normalizerequest(input) { return Object.freeze({ capabilities: Object.freeze(unique(input.capabilities ?? [], "provider chain requested capability")), mincpu: nonnegative(input.mincpu ?? 0, "provider chain minimum cpu"), minmemorybytes: nonnegative(input.minmemorybytes ?? 0, "provider chain minimum memorybytes"), minmilliseconds: nonnegative(input.minmilliseconds ?? 0, "provider chain minimum milliseconds") }); }
function eligibility(provider, request) { const reasons = []; if (provider.status !== "available") reasons.push(`status:${provider.status}`); for (const capability of request.capabilities) if (!provider.capabilities.includes(capability)) reasons.push(`capability:${capability}`); if (provider.cpu < request.mincpu) reasons.push("cpu"); if (provider.memorybytes < request.minmemorybytes) reasons.push("memorybytes"); if (provider.maxmilliseconds < request.minmilliseconds) reasons.push("maxmilliseconds"); return reasons; }
function describe(provider) { return Object.freeze({ id: provider.id, priority: provider.priority, capabilities: Object.freeze([...provider.capabilities]), cpu: provider.cpu, memorybytes: provider.memorybytes, maxmilliseconds: provider.maxmilliseconds }); }
